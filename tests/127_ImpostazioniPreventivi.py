...
class Impostazioni(Test):
    ...
    def condizioni_fornitura_preventivi(self):
        ...
        testo = self.wait_driver.until(
            EC.visibility_of_element_located((By.XPATH, '(//div[@id="viewer"]//span)[65]'))
        ).text   #controlla se la condizione coincide con quella messa in impostazioni
    ...
    def esclusioni_preventivi(self):
        ...
        testo = self.wait_driver.until(
            EC.visibility_of_element_located((By.XPATH, '//textarea[@id="esclusioni"]'))
        ).text  #check se non è stato aggiunto nessun testo
        ...
        testo = self.wait_driver.until(
            EC.visibility_of_element_located((By.XPATH, '//textarea[@id="esclusioni"]'))
        ).text  #check se è stato aggiunto il testo
        ...
        testo = self.wait_driver.until(
            EC.visibility_of_element_located((By.XPATH, '//textarea[@id="setting205"]'))
        )   #cancello testo
        testo.clear()
